...
function Wizard({profile,onSaved}){
  ...
  return <div className="stack repair-wizard-wrap">
    ...
    <section className="wizard-panel">
      ...
      {step===3&&<>
        <CardTitle icon="alert" title={isIJInjection?"Failure Point & Symptom":"เลือกจุดเสียและอาการ"} sub={isIJInjection?`เลือกจุดเสียและอาการ · ${selectedMachine?.machine_no||"-"} · ${selectedMachine?.machine_name||""}`:`${selectedMachine?.machine_no||"-"} · ${selectedMachine?.machine_name||""}`}/>
        <div className="wizard-context-banner">
          <div className="context-chip"><span>{isIJInjection?"Machine Zone · กลุ่มเครื่อง":"กลุ่มเครื่อง"}</span><b>{selectedGroup?.group_name||"-"}</b></div>
          <div className="context-chip"><span>{isIJInjection?"Machine No. · หมายเลขเครื่อง":"หมายเลขเครื่อง"}</span><b className="mono">{selectedMachine?.machine_no||"-"}</b></div>
          <div className="context-chip"><span>{isIJInjection?"Production Line · ไลน์ผลิต":"ไลน์ผลิต"}</span><b>{selectedMachine?.production_line||"-"}</b></div>
        </div>
        {isIJInjection?<div className="analysis-select-grid compact-select-grid ij-structured-master">
          <section className="choice-panel soft">
            <div className="choice-panel-head compact"><div><h4>Zone / Machine System <span className="req">*</span></h4><p>โซน / ระบบของเครื่อง · เลือกตามโครงสร้างเครื่อง Injection ของแผนก IJ</p></div><span className="choice-count mono">{ijZones.length} Systems</span></div>
            <div className="field"><SearchSelect className="professional-select" value={form.ij_zone_code} onChange={v=>setForm(f=>({...f,ij_zone_code:v,ij_component_code:"",ij_symptom_code:"",ij_mold_model:""}))} placeholder="Select Zone / Machine System" searchPlaceholder="Search system / ค้นหาระบบ…" options={ijZones.map(z=>({value:z.code,label:z.name_en,sub:z.name_th}))}/></div>
            {selectedIJZone&&<div className="selected-preview-card"><span>Selected System · ระบบที่เลือก</span><b>{selectedIJZone.name_en}</b><small>{selectedIJZone.name_th}</small></div>}
          </section>
          <section className="choice-panel soft">
            <div className="choice-panel-head compact"><div><h4>Component / Failure Point <span className="req">*</span></h4><p>อุปกรณ์ / จุดเสีย · ตัวเลือกจะเปลี่ยนตามระบบที่เลือก</p></div><span className="choice-count mono">{visibleIJComponents.length} Items</span></div>
            <div className="field"><SearchSelect className="professional-select" value={form.ij_component_code} onChange={v=>setForm(f=>({...f,ij_component_code:v,ij_symptom_code:"",ij_mold_model:""}))} disabled={!form.ij_zone_code} placeholder={form.ij_zone_code?"Select Component / Failure Point":"Select system first / เลือกระบบก่อน"} searchPlaceholder="Search component / ค้นหาอุปกรณ์…" options={visibleIJComponents.map(c=>({value:c.code,label:c.name_en,sub:c.name_th}))}/></div>
            {selectedIJComponent&&<div className="selected-preview-card"><span>Selected Component · จุดที่เลือก</span><b>{selectedIJComponent.name_en}</b><small>{selectedIJComponent.name_th}</small></div>}
          </section>
          <section className="choice-panel soft full-width-ij-symptom">
            <div className="choice-panel-head compact"><div><h4>Failure Symptom <span className="req">*</span></h4><p>อาการเสีย · ใช้ Master กลางของ IJ เพื่อให้ข้อมูลวิเคราะห์ต่อได้ตรงกัน</p></div><span className="choice-count mono">{ijSymptoms.length} Symptoms</span></div>
            ...
            }} disabled={!form.ij_component_code} placeholder={form.ij_component_code?"Select Failure Symptom":"Select component first / เลือกอุปกรณ์ก่อน"} searchPlaceholder="Search symptom / ค้นหาอาการ…" options={ijSymptoms.map(x=>({value:x.code,label:x.name_en,sub:x.name_th}))}/></div>
            {selectedIJSymptom&&<div className={`selected-preview-card accent ${selectedIJSymptom.requires_mold_model?"material-leak":""}`}><span>Selected Symptom · อาการที่เลือก</span><b>{selectedIJSymptom.name_en}</b><small>{selectedIJSymptom.name_th}</small></div>}
            {isIJMaterialLeak&&<>
      ...
      {step===4&&<>
        <CardTitle icon="repair" title="วิเคราะห์สาเหตุและการแก้ไข" sub="ข้อมูลส่วนนี้ใช้ต่อในประวัติซ่อม, Pareto และการติดตามงาน"/>
        <div className="analysis-layout">
          ...
          {!freeCauseAction?<div className="analysis-select-grid"><section className="choice-panel soft"><div className="choice-panel-head compact"><div><h4>สาเหตุ <span className="req">*</span></h4><p>เลือกสาเหตุที่ Admin กำหนดไว้สำหรับเครื่องนี้</p></div></div>{!machineCauses.length?<Empty title="ยังไม่มีสาเหตุสำหรับเครื่องนี้" text="Admin ยังไม่ได้กำหนด Cause ให้เครื่องนี้ จึงไม่มีตัวเลือกและไม่สามารถพิมพ์เองได้"/>:<div className="field"><SearchSelect className="professional-select" value={form.cause_id} onChange={v=>patch("cause_id",v)} placeholder="เลือกสาเหตุ" searchPlaceholder="พิมพ์ค้นหาสาเหตุ…" options={machineCauses.map(c=>({value:c.id,label:c.cause_name,sub:c.category||c.cause_code||""}))}/></div>}</section><section className="choice-panel soft"><div className="choice-panel-head compact"><div><h4>วิธีแก้ไข <span className="req">*</span></h4><p>เลือกวิธีแก้ไขที่ Admin กำหนดไว้สำหรับเครื่องนี้</p></div></div>{!machineActions.length?<Empty title="ยังไม่มีวิธีแก้ไขสำหรับเครื่องนี้" text="Admin ยังไม่ได้กำหนด Action ให้เครื่องนี้ จึงไม่มีตัวเลือกและไม่สามารถพิมพ์เองได้"/>:<div className="field"><SearchSelect className="professional-select" value={form.action_id} onChange={v=>patch("action_id",v)} placeholder="เลือกวิธีแก้ไข" searchPlaceholder="พิมพ์ค้นหาวิธีแก้ไข…" options={machineActions.map(a=>({value:a.id,label:a.action_name,sub:a.action_code||""}))}/></div>}</section></div>:<div className="field-grid cols-2" style={{marginTop:18}}><div className="field"><label>{isIJInjection?<>Cause <small>สาเหตุ</small> <span className="req">*</span></>:<>สาเหตุ <span className="req">*</span></>}</label><textarea className="textarea" value={form.cause} onChange={e=>patch("cause",e.target.value)} placeholder={isIJInjection?"Type cause / พิมพ์สาเหตุที่วิเคราะห์ได้":"สาเหตุที่วิเคราะห์ได้"}/></div><div className="field"><label>{isIJInjection?<>Corrective Action <small>วิธีแก้ไข</small> <span className="req">*</span></>:<>วิธีแก้ไข <span className="req">*</span></>}</label><textarea className="textarea" value={form.action_taken} onChange={e=>patch("action_taken",e.target.value)} placeholder={isIJInjection?"Type corrective action / พิมพ์วิธีการแก้ไข":"วิธีการแก้ไขที่ดำเนินการ"}/></div></div>}
          <div className="analysis-note"><Icon name="warning" size={17}/><span>เลือกสาเหตุและวิธีแก้ไขให้ตรงกับงานจริง เพื่อให้ข้อมูลสามารถนำไปใช้ต่อในการวิเคราะห์ Pareto และติดตามปัญหาได้อย่างถูกต้อง</span></div>
        </div>
      </>}
      {step===5&&<>
      ...
      {step===6&&<>
        ...
        <div className="review-card premium">
          ...
          <div className="review-grid two-col">
            ...
            <div><span>{isIJInjection?"System / Component · ระบบ / จุดเสีย":"จุดเสีย"}</span><b>{isIJInjection?`${selectedIJZone?.name_en||"-"} › ${selectedIJComponent?.name_en||"-"}`:(selectedPoint?.point_name||form.area_point_text||"-")}</b>{isIJInjection&&<small>{selectedIJZone?.name_th||"-"} › {selectedIJComponent?.name_th||"-"}</small>}</div><div><span>{isIJInjection?"Symptom · อาการ":"อาการ"}</span><b>{isIJInjection?(selectedIJSymptom?.name_en||"-"):(selectedProblem?.problem_name||form.symptom||"-")}</b>{isIJInjection&&<small>{selectedIJSymptom?.name_th||"-"}</small>}</div>
...
function ReportDetail({row,profile,onClose,onChanged}){
  const admin=profile.role==="admin",supervisor=profile.role==="supervisor";
  const canManage=admin||(!supervisor&&row.technician_id===profile.id);
